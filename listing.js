// api/listing.js  (build: og-preview-v2)
...
export default async function handler(req, res) {
  ...
  try {
    const q = `${SB_URL}/rest/v1/listings?select=address,city,state,title,status,price,sold_price,hero_image,gallery,beds,baths,sqft&slug=eq.${encodeURIComponent(slug)}&published=is.true&limit=1`;
    ...
    if (l) {
      ...
      const metaDesc = [l.title, specs, priceTxt, [l.city, l.state].filter(Boolean).join(", ")]
        .filter(Boolean).join(". ") + ". Listed by Ralph Richardson, KW Corona.";
